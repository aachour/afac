                ...
                <div class="row align-items-stretch g-0 no-gutters">';
                    if ($entry->type_id <= 5) {
                        ...
                        $html .= '<div class="col-12">';
                            ...
                            $html .= '<div class="mt-2 mb-3 micro black">' . (app()->getLocale() == 'en' ? $entry->image_caption : $entry->image_caption_arabic) . '</div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 col-12">';
                            if ($entry->type_id == 3) { //Supported Project
                                ...
                                $html .= '<div class="mt-1"><a href="' . route('entry.view', ['entryType' => 'program', 'id' => $entry->programYears?->programYear?->program?->id]) . '" class="medium black ABCDiatypeMedium">' . (app()->getLocale() == 'en' ?  $entry->programYears?->programYear?->program?->program_title :  $entry->programYears?->programYear?->program?->program_title_arabic) . '</a></div>';
                                ...
                                foreach ($projectGrantees as $projectGrantee) {
                                    $html .= '<div class="mt-1"><a href="' . route('entry.view', ['entryType' => 'project', 'id' => $projectGrantee->project->id]) . '" class="medium black ABCDiatypeMedium">' . (app()->getLocale() == 'en' ?  $projectGrantee->project?->project_title :  $projectGrantee->project?->project_title_arabic) . '</a></div>';
                                }
...
function ViewCollection($collection_id)
{
    ...
    if (($show_name == 1 || $show_description == 1) && $featured_width != '74.3%') {
        $html .= '<div class="titleDescription mb-3 mb-lg-0">';
        if ($show_name == 1) {
...
function ViewPattern($section_column_id)
{
    ...
    if ($column) {
        ...
        $section = \App\Models\Sections::with('bgColor')->find($column->section_id);
        $overlayFill = ($section && $section->bgColor && !empty($section->bgColor->code)) ? $section->bgColor->code : 'transparent';
...
function getEntryLabels($entry)
{
    ...
    if ($entry->type_id == 1) {
        ...
        if ($end_timestamp < $current) {
            $labels[] = app()->getLocale() == 'en' ? 'Closed for applications' : 'مغلق للطلبات';
        } else {
            ...
            if ($entries_selection == 2) {
                $html .= '<div class="sort">
                    <select class="filterDpd sortDpd" id="sort-collection-' . $collection_id . '">
                        <option value="">' . ($isArabic ? 'الترتيب' : 'Sort by') . '</option>
                        <option value="1">' . ($isArabic ? 'الاسم' : 'Name') . '</option>';
                        //<option value="2">' . ($isArabic ? 'الاسم تنازلي' : 'Name DESC') . '</option>
                        //<option value="3">' . ($isArabic ? 'التاريخ تصاعدي' : 'Date ASC') . '</option>
                        $html .= '<option value="4">' . ($isArabic ? 'التاريخ' : 'Date') . '</option>
                    </select>
                </div>';
            }
            $html .= '<div class="clear"></div>
        ...
        $html .= '<div class="filters" style="">
            ...
            <div class="filter">
                <select class="filterDpd filter_project_category">
                    <option value="">' . ($isArabic ? 'الموضوع' : 'Ttheme') . '</option>';
            ...
            <div class="filter">
                <select class="filterDpd filter_project_program">
                    <option value="">' . ($isArabic ? 'البرنامج' : 'Program') . '</option>';
        ...
        $html .= "<script>
                $(document).ready(function(){
                    ...
                    $('#filter-collection-" . $collection_id . "').click(function () {
                        ...
                        project_program_year_" . $collection_id . "=$(parent).find('.filter_project_program_year').val();
        ...
        $html .= '<div class="filters" style="">
            ...
            <div class="filter">
                <select class="filterDpd filter_grantee_category">
                    <option value="">' . ($isArabic ? 'الموضوع' : 'Theme') . '</option>';
        ...
        $html .= '<div class="filters" style="">
            <div class="filter">
                <select class="filterDpd filter_resource_category">
                    <option value="">' . ($isArabic ? 'الفئة' : 'Category') . '</option>';
...
function buildEntriesQuery($collection_id = "", $filters = "", $entries_id = [])
{
    ...
    if (!empty($sort)) {
        ...
        if ($entries_order == 1) //event name asc
        {
            ...
            if ($collection_type_id == 7) {
                $query->orderBy('news_title', 'asc');
            }
            if ($collection_type_id == 8) {
                $query->orderBy('external_title', 'asc');
            }
            if ($collection_type_id == 9) {
                $query->orderBy('team_name', 'asc');
            }
            if ($collection_type_id == 10) {
                $query->orderBy('board_name', 'asc');
            }
...
function getMenuPages(){
    ...
    $pages=Pages::WHERE(['in_menu'=>'1','published'=>'1'])->orderby('list_order','asc')->get();
    return $pages;
